...
var Interna = {
	...
	init : function() {
        ...
        $('#palhetaDireitaAba #botao-importar').click(function(){
            
           Base.JanelaFlutuante.load(baseUrlController + "/importar/"); 
		...
		if(no_action == "novo") {
            ...
            $('#usuario_inicial').blur(function() {
                if($(this).val()=='')
                    $("#usuario_inicial").val("");
            });
        } else if(no_action == "salvar") {
            
            if($("#importou").val() == "1") {
                
                // Atualiza a aba
                window.parent.Abas.atualizaIframe("ifrm_0");
                
                // fechar a janela e recarregar a aba inferior
                window.parent.Base.JanelaFlutuante.close();
                
            } else {
                // Mostra a mensagem de erro
                Base.montaMensagemSistema(Array("Não foi possível importar grupos."), "ALERTA", 4, "window.location.href = '" + baseUrlController + "/importar/';");
            }
	...
	retornaGrupoX : function(indice, cd_grupo) {
		...
		, function (retorno){ // Pega o retorno 
			...
			if(indice >= 0) {
				...
				$('#ds_grupo_' + indice).val(retorno.DS_GRUPO);
				
			} else {
				$('#cd_grupo').val(retorno.CD_GRUPO);
				$('#ds_grupo').val(retorno.DS_GRUPO);
			}
    ...
    /*atualizaAba : function() {
        
        window.parent.Interna.atualizaAba("#ifrm_0");
    }*/
};