	...
	public function menu() {
		...
		try {
			...
			foreach ($menuSistemas as $chave => $sistema) {
                ...
                if(isset($sessao->perfil->CD_GRUPO) && trim($sistema->NO_SISTEMA) == "Documentos" && 
                    (($sessao->perfil->TP_USUARIO == "P" && in_array("91", $sessao->perfil->CD_GRUPO)) || 
                    ($sessao->perfil->FL_SUPER == 1) && in_array("94", $sessao->perfil->CD_GRUPO)) && 
                    $sessao->perfil->CD_USUARIO != "PUBLICO") {
            ...
            // Inicio do bloco com os Menus e as Transações
            if(! empty($params['_cd_sistema'])) {
	    		...
	    		$cd_menu = 0;
	    		foreach($abasMenuSistema as $indice => $linha) {
					...
					if (strtoupper(trim($linha->AMB_DESENV)) == 'ZEND') {
						...
						$parametros = array("_cd_sistema"        => $params['_cd_sistema'], 
                                            ...
                                            "_fl_tipo_transacao" => 'M', // todas as telas selecionadas pelo menu são Master
                                            "_fl_permissao"      => $linha->FL_PERMISSAO);
